...
class ConnectHandler(RequestPacketHandler):
    ...
    @asyncio.coroutine
    def build_request_from_session(self, session: Session) -> ConnectPacket:
    ...
    @asyncio.coroutine
    def _decode_payload(self, fixed: MQTTHeader, variable_header: ConnectVariableHeader, reader: asyncio.StreamReader):
        payload = ConnectPayload()
        #  Client identifier
        try:
